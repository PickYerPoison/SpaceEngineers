...
public void Main(string argument, UpdateType updateSource)
{
    ...
    else if (updateSource == UpdateType.Antenna && argument != "")
    {
        ...
        if (messageData.Valid)
        {
            ...
            {
                case MessageOrders.DTM_STATUS:
                case MessageOrders.DTM_DISTRESS:
                    Commands.Enqueue(new Command(messageData.Order, messageData.Data[0], messageData.Sender));
                    break;
                case MessageOrders.DTM_ORE_INFO:
                    Commands.Enqueue(new Command(messageData.Order, messageData.Data[0], 
                                     new Vector3(messageData.Data[1], messageData.Data[2], messageData.Data[3])));
                    break;
                default:
                    break;
            }
        ...
        if (Commands.Count() > 0)
        {
            ...
            switch (Commands.Peek().Order)
            {
                case MessageOrders.DTM_STATUS:
                    ...
                    Echo("Drone status report.");
                    completedCommand = true;
                    break;
                case MessageOrders.DTM_DISTRESS:
                    // register distress call
                    Echo("Drone distress call.");
                    completedCommand = true;
                    break;
                case MessageOrders.DTM_ORE_INFO:
                    // update ore information
                    Echo("Ore information report.");
                    completedCommand = true;
                    break;
                default:
                    break;
